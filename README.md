...
- **新增**：多模态理解（图片/音频/视频识别）
- **新增**：消息引用处理（支持引用历史图片/视频/音频/文本）
- **新增**：历史文件引用（关键词匹配 + 时间戳匹配）
- **新增**：视频 URL 自动提取和分析
- **新增**：友好提示（缺少模型配置时显示当前可用功能）
- **新增**：长回复分段发送和摘要缓存
- **新增**：`replies.py` 集中管理所有用户可见回复
- **新增**：危险命令安全拦截（删除/注入/系统命令检测）
- **优化**：LLM 引擎支持多模型自动切换
- **优化**：流式输出支持（qwen3.5-omni-plus）
- **优化**：system 消息自动合并（兼容多模态模型）
- **优化**：AES 解密支持 ECB/CBC 双模式
- **优化**：文件过期自动清理

### v1.0 (2026-05-07)

- 初始版本：文本对话、文档解析、对话记忆、技能系统
