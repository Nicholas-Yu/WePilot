...
微信官方已推出 ClawBot 插件，让 OpenClaw 可以直接在微信内使用。OpenClaw 作为功能完善的 Agent 框架，迭代速度快、能力丰富，适合有一定技术背景的用户深度使用。

WePilot 从本人真实需求出发，选择做一个**轻量级的微信 AI 助手**，专注于：

- **文档处理**：在微信中快速解析和分析各类文档
- **数据分析**：简单实用的数据分析和摘要功能
- **多模态理解**：按需处理图片、音频、视频等多模态数据

这些能力可以覆盖普通用户 **80% 的日常使用场景**。不需要复杂的框架和沉重的依赖，开箱即用，稳定可靠。
...
- **技能交互菜单**：技能可定义交互式菜单（如报告类型选择），触发后即时推送，用户选择后自动回放原始请求
- **消息去重**：基于消息 ID 的 5 分钟 TTL 去重 + 媒体内容指纹去重（纯文本消息不参与内容去重），防止平台重复投递
...
| 技能 | 功能 | 特色 |
|------|------|------|
| `industry-investment-analysis` | 投研分析/行业研究/个股分析 | 交互式报告类型菜单（6 种报告模板） |
| `media-data-analyst` | 跨渠道媒体数据分析 | 支持 xlsx/xls/csv 数据表格 |
...
enabled: true
menu: "请选择报告类型：\n1. 快速简报\n2. 标准报告\n3. 深度研报"
menu_keywords: ["快速看看=快速简报", "深度分析=深度研报", "standard=标准报告"]
---
...
技能可以通过 `menu` 和 `menu_keywords` 字段实现交互式两步流程：

1. **触发阶段**：用户消息匹配到技能后，系统立即发送 `menu` 定义的菜单文本，同时保存原始消息和文件上下文
2. **选择阶段**：用户回复菜单选项（支持数字 `1`-`6`、自然语言 `选1`/`第一个`/`我要2`、或 `menu_keywords` 中的关键词），系统回放原始消息并注入选定的报告类型
...
用户: "选2" 或 "标准报告"
  → 回放原始消息 + 注入报告类型 → LLM 按选定模板输出
```

**menu_keywords 格式**：`关键词=报告类型`，关键词匹配不区分大小写，报告类型保留原始大小写。

**自动跳过菜单的场景**：
- 用户消息中已包含 `menu_keywords` 中的关键词（如"深度分析 XX"）
- 附件超时自动分析（auto-analyze）场景，默认使用"快速简报"
...
├── user_skills/            # 用户自定义技能
│   ├── industry-investment-analysis/  # 投研分析（含交互式菜单）
│   └── media-data-analyst/ # 媒体数据分析
├── tests/                  # 测试
│   └── test_bugfixes.py    # Bug 修复验证测试
└── data/
    ├── bot.pid             # 进程锁
    ├── bot.log             # 运行日志
...
```
1. 接收消息 → 去重检查（消息 ID + 媒体内容指纹）
2. 解析消息 → 提取文本、附件、引用内容
3. 菜单选择检测 → 如有待确认菜单，尝试匹配用户回复
   ├─ 匹配成功 → 回放原始消息 + 注入报告类型 → 跳到步骤 7
   └─ 未匹配 → 保留菜单，继续正常流程
4. 下载附件 → 流式下载 + AES 解密
5. 分析文件 → 文档解析 / 多模态编码（base64）
6. 历史引用 → 关键词匹配 / 时间戳匹配
7. 构建上下文 → 对话摘要 + 活跃文件 + 历史记录
8. 选择技能 → 意图匹配 + 文件类型匹配
   ├─ 技能有菜单 + 无预选类型 → 发送菜单，保存上下文，等待用户选择
   └─ 技能无菜单 / 已有预选类型 → 注入技能上下文
9. 调用 LLM → 自动选择模型（文本/图片/音频/视频）
10. 处理回复 → 分段发送 / 摘要缓存
11. 记录记忆 → 更新对话历史 + 活跃文件
```
...
当用户明确指代历史文件时，系统会自动加载：
- **强匹配**：包含明确的文件指代短语，如"这个文件"、"刚才的视频"、"之前发的"、"那个PPT"
- **弱匹配**：同时包含指代词（这个/那个/刚才…）和文件类型词（文件/文档/报告/PDF/Excel…）

> **注意**：如果用户显式引用了消息（有 `ref_msg`），则不会触发历史文件猜测。普通对话中的"看下"、"分析"、"总结"等词不会误触发历史文件加载。

## 测试

```bash
# Bug 修复验证测试（菜单系统、去重、TTL 等）
